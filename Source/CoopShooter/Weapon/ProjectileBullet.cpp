...
{	
	ACoopCharacterBase* OwnerCharacter = Cast<ACoopCharacterBase>(GetOwner());
	if (OwnerCharacter)
	{
		ACoopPlayerController* OwnerController = Cast<ACoopPlayerController>(OwnerCharacter->Controller);
		if (OwnerController)
		{
			if (OwnerCharacter->HasAuthority() && !bUsedServerSideRewind)
			{
			...
			ACoopCharacterBase* HitCharacter = Cast<ACoopCharacterBase>(OtherActor);
